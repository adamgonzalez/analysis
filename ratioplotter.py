...
#### jus the xmm and suz fits to show the iron line differences
...
#    if (i == 4):
#        shape = 's'
#    plottertron(dq["e{0}".format(x)], dq["e_err{0}".format(x)], dq["f{0}".format(x)], dq["f_err{0}".format(x)], c_set[i], n_set[i], 1, 2, 'marker', shape)
...
#        ax.set_ylim(0.75,1.3)
    ...
    for i in range(0,2):
        ...
        if (i == 0):
            ax.text(.3,.1,'cutoffpl',horizontalalignment='center',transform=ax.transAxes)
        elif (i == 1):
...
#plt.savefig('/Users/agonzalez/Dropbox/Graduate/PhD/IIIZw2Paper2017/xmm+suz_ironline.png', bbox_inches='tight', dpi=300)