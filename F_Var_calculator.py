...
swift = np.genfromtxt('rbn_spectra/xrt_july.lc', skip_header=2)
...
ax11.axhline(y=xmmavg, color='k', dashes=[5,3], linewidth=1)
...
ax12.axhline(y=suzavg, color='k', dashes=[5,3], linewidth=1)
...
# plt.savefig('rbn_spectra/xmm_suz_lightcurves_zoomout.png', bbox_inches='tight', dpi=300)
plt.show()
...
# suzmjd[:,2] = suz[:,2]*0.5094/2.0
# suzmjd[:,3] = suz[:,3]*0.5094/2.0
...
#
#
# total_lc = [[swift[0,0],swift[0,1],swift[0,3],swift[0,4]]]
# for i in range (1,len(swift)):
#     total_lc = np.concatenate((total_lc, [[swift[i,0],swift[i,1],swift[i,2],swift[i,3]]]))
# for i in range (0,len(xmmmjd)):
#     total_lc = np.concatenate((total_lc, [[xmmmjd[i,0],xmmmjd[i,1],xmmmjd[i,2],xmmmjd[i,3]]]))
# for i in range (0,len(suzmjd)):
#     total_lc = np.concatenate((total_lc, [[suzmjd[i,0],suzmjd[i,1],suzmjd[i,2],suzmjd[i,3]]]))
# # total_lc = np.concatenate((total_lc, [[xmmmjdT, e_xmmmjdT, xmmmjdavg, e_xmmmjdavg]]))
# # total_lc = np.concatenate((total_lc, [[suzmjdT, e_suzmjdT, suzmjdavg, e_suzmjdavg]]))
#
#
# # print out all of the average count rates for each telescope and the error on the mean
...
#
# plt.figure()
# plt.axhline(y=wm, color='k', dashes=[5,3], linewidth=1)
...
# plt.errorbar(x=suzmjdT, y=suzmjdavg, xerr=e_suzmjdT, yerr=e_suzmjdavg, fmt='o', markersize=7, ecolor='r', capthick=1, color='r', linewidth=1.0, label='Suzaku', markerfacecolor='none')
# plt.errorbar(x=swift[:,0], y=swift[:,3], xerr=swift[:,1], yerr=swift[:,4], fmt='s', markersize=7, ecolor='b', capthick=1, color='b', linewidth=1.0, label='Swift', markerfacecolor='none')
# plt.ylim(0.,1.)
# plt.xlabel('Time (MJD)')
...
#
# # plt.savefig('rbn_spectra/all_lightcurves_swiftflux.png', bbox_inches='tight', dpi=300)
# plt.show()
