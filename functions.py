...
def psdbin2(factor, xvar, yvar):
    ...
    # bin_index, data_counter = 0, 0
    #
    # for i in range (data_counter, n-1):
    #     yvar_sum = 0.0
    #     sum_counter = 0
    #
    #     ### bin up all of the stuff within 1.3x the initial bin frq
    #     for j in range (data_counter, n):
    #         if (xvar[i] <= xvar[j] < 1.3*xvar[i]):
    #             yvar_sum += yvar[j]
    #             sum_counter += 1
    #             data_counter += 1
    #
    #     ### if the 1.3xfi bin only had 1 point then do the next point averaged with the first
    #     if (sum_counter < 2):
    #         yvar_bin = np.append(yvar_bin, [[(yvar[i] + yvar[i+1])/2.0]])
    #         yerr_bin = np.append(yerr_bin, [[((yvar[i] + yvar[i+1])/2.0)/np.sqrt(2.0)]])
    #         data_counter += 1
    #     ### otherwise, just do regular binning methods
    #     elif (sum_counter >= 2):
    #         yvar_bin = np.append(yvar_bin, [[yvar_sum/sum_counter]])
    #         yerr_bin = np.append(yerr_bin, [[(yvar_sum/sum_counter)/np.sqrt(sum_counter)]])
    #
    #     xvar_bin = np.append(xvar_bin, [[xvar[i]]])
    #     bin_index += 1
    ...
    return xvar_bin, yvar_bin[1:], yerr_bin[1:]
###################################################################################################
###################################################################################################
